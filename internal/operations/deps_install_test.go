...
func TestRunPostCreateHooksEmpty(t *testing.T) {
	tmpDir := t.TempDir()
	results := RunPostCreateHooks(context.Background(), tmpDir, nil, nil, nil)
	if len(results) != 0 {
...
func TestRunPostCreateHooksInvalidCommand(t *testing.T) {
	tmpDir := t.TempDir()
	results := RunPostCreateHooks(context.Background(), tmpDir, []string{"nonexistent-command-xyz"}, nil, nil)
	if len(results) != 1 {
