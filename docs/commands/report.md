...
**What gets recorded.** Observability is **on by default**. The log stream (`.log.jsonl`, written to the same day-files; read by `rimba report` only to extract the rimba version(s) seen, never its command-line contents) records every git/gh/exec/hook subprocess this repo's rimba commands run, including the full command line and, on failure, the subprocess's captured stderr/output — so a `rimba exec` command or a `post_create`/dependency-install command that embeds a credential (e.g. a registry token or a git URL with an embedded password), or whose failure output echoes one, is written verbatim to `~/Library/Caches/rimba` (or the platform-equivalent cache dir) and kept for `retention_days` (default 14). Files are created `0600` (owner-only), but that doesn't protect against home-directory backups or cache-syncing tools. If this matters for your workflow, set `[observability] enabled = false` in `.rimba/settings.toml`, or export `RIMBA_NO_OBSERVABILITY` to disable it for a single invocation.
